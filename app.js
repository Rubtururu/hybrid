...
async function loadStats() {
    ...
    try {
        const [tvl, divPool, users, totalShares, volume, totalDeposited, totalWithdrawn, rafflePot, lastDividendTime] =
        await Promise.all([
        ...
        // Guardamos next dividend
        nextDividendTime = parseInt(lastDividendTime) + 24*3600;
        ...
        const userSharesPercent = totalShares > 0 ? u.shares / totalShares * 100 : 0;
        const dailyDividend = divPool / 100; // 1% diario
        ...
        document.getElementById("sharePercent").innerText = userSharesPercent.toFixed(2) + "%";
        document.getElementById("nextDividend").innerText = web3.utils.fromWei(dailyDividend.toString());
...
async function loadRafflePlayers() {
    ...
    for(let i=0;i<Math.min(5,totalUsers);i++){
        ...
        if(p){
            ...
            li.innerText = `${p} - Pot Share ${(rafflePot/totalUsers).toFixed(6)} BNB`;
            ul.appendChild(li);
        }
    }
}
...
// ---------- DIVIDEND COUNTDOWN ----------
function startDividendCountdown(){
    const el = document.getElementById("divCountdown");
    setInterval(()=>{
