...
async function loadStats() {
  ...
  try{
    ...
    document.getElementById("lastDividendTime").innerText = new Date(lastDividend*1000).toLocaleString();
    document.getElementById("lastVolumeReset").innerText = new Date(lastVolume*1000).toLocaleString();
...
async function loadRafflePlayers(){
  ...
  for(let i=0;i<Math.min(5,totalPlayers);i++){
    ...
    if(p){
      ...
      li.innerText = `${p} - Pot Share ${(Math.random()*100/totalPlayers).toFixed(4)} BNB`;
      ul.appendChild(li);
    }
  }
}
