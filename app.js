...
let web3, contract, account, chart, nextDividendInterval;
...
async function loadStats() {
  ...
  try {
    const [tvl, divPool, users, totalShares, volume, totalDeposited, totalWithdrawn, rafflePot, lastDividend] =
      await Promise.all([
    ...
    document.getElementById("totalUsers").innerText = parseInt(users);
    ...
    // Next dividend = 1% del dividend pool
    const nextDividend = web3.utils.fromWei(divPool) * 0.01;
    document.getElementById("nextDividend").innerText = parseFloat(nextDividend).toFixed(6) + " BNB";

    // Usuario stats
    const u = await contract.methods.users(account).call();
    const userShares = parseFloat(web3.utils.fromWei(u.shares));
    const sharePercent = totalShares > 0 ? (userShares / parseFloat(web3.utils.fromWei(totalShares)) * 100).toFixed(2) : 0;
    ...
    document.getElementById("shares").innerText = userShares;
...
async function startDividendCountdown(){
  clearInterval(nextDividendInterval);
  const lastDividend = parseInt(await contract.methods.lastDividendTime().call());
  const now = Math.floor(Date.now()/1000);
  const nextPay = lastDividend + 24*3600;
  nextDividendInterval = setInterval(()=>{
    const diff = nextPay - Math.floor(Date.now()/1000);
    if(diff<=0){
      document.getElementById("dividendCountdown").innerText = "0h 0m 0s";
    } else {
      const h=Math.floor(diff/3600);
      const m=Math.floor((diff%3600)/60);
      const s=diff%60;
      document.getElementById("dividendCountdown").innerText = `${h}h ${m}m ${s}s`;
    }
  },1000);
}
