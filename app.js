...
let web3, contract, account, chart, dividendTimer;
...
async function loadStats(){
    ...
    try{
        ...
        // % de share
        let sharePercent = totalShares>0 ? (u.shares/totalShares*100).toFixed(2) : 0;
        ...
        // Next dividend (1% del pool)
        const nextDiv = div/100;
        document.getElementById("nextDividend").innerText = web3.utils.fromWei(Math.floor(nextDiv).toString());
...
// ---------- DIVIDEND COUNTDOWN ----------
function startDividendCountdown(lastDividend){
    clearInterval(dividendTimer);
    dividendTimer = setInterval(async ()=>{
        const now = Math.floor(Date.now()/1000);
        let next = parseInt(lastDividend) + 86400; // 24h
        let diff = next - now;
        if(diff<0) diff=0;
