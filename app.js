...
let web3, contract, account, chart, dividendCountdown;
...
async function loadStats() {
    ...
    try {
        ...
        document.getElementById("users").innerText = parseInt(totalUsers);
        ...
        // % de share
        const userSharePercent = totalShares == 0 ? 0 : (u.shares / totalShares * 100);
        document.getElementById("sharePercent").innerText = userSharePercent.toFixed(2);

        // Dividendos diarios 1%
        const dailyDividends = divPool * 0.01;
        document.getElementById("nextDividend").innerText = web3.utils.fromWei(dailyDividends.toString());

        // Cuenta atrás 24h
        startDividendCountdown(lastDividend);

        // Actualiza gráfico
        updateChart(tvl, divPool);

        // Actualiza jugadores rifa
        loadRafflePlayers();
    } catch(err) {
...
// ---------------- DIVIDEND COUNTDOWN ----------------
function startDividendCountdown(lastDividendTime) {
    const nextDividendTime = parseInt(lastDividendTime) + 24*3600; // 24h
    clearInterval(dividendCountdown);
    dividendCountdown = setInterval(()=>{
...
async function loadRafflePlayers() {
    ...
    for(let i=0; i<Math.min(5,totalUsers); i++) {
        const player = await contract.methods.rafflePlayers(i).call().catch(()=>null);
        if(player){
